...
| [agent-skills](https://github.com/conorbronsdon/agent-skills) | Production-tested skills for Claude Code, Codex, Cursor, and OpenCode — session management, multi-session reconciliation, worktree recovery, PR review, and skill scaffolding. The consolidated successor to claude-code-skills. | [@conorbronsdon](https://github.com/conorbronsdon) |
| [claude-context-os](https://github.com/conorbronsdon/claude-context-os) | An operating system for your Claude context — version-controlled files, a self-curating memory, and a /start→/end session loop. Template repo; works in Claude Code and claude.ai. | [@conorbronsdon](https://github.com/conorbronsdon) |

### Development & Code Review

| Skill | What it does | Author |
|-------|-------------|--------|
| [code-review](https://github.com/conorbronsdon/agent-skills/tree/main/code-review) | Multi-agent PR review — orchestrates Copilot + parallel subagents (adversarial, operational, reference-comparison) sized to PR risk. Catches architectural P0s that single-pass review misses. | [@conorbronsdon](https://github.com/conorbronsdon) |
...
| [Typefully](https://typefully.com) | Hosted drafting and scheduling for X, LinkedIn, Threads, Bluesky, Mastodon, and Substack Notes, with an [official REST API, hosted MCP, and Agent Skill](https://typefully.com/ai-agents). Its OAuth connections live with the service, so the per-platform app-review gating below stops being your problem: an agent drafts through the API and your connected accounts publish on schedule. Planned drafts sit on the calendar inert until confirmed, which makes a clean review gate for agent-written copy, and media upload works over the API (tested with 15-25MB video clips). Publish volume is tier-gated. | [Typefully](https://typefully.com) |
